---
id: errors
title: Error hierarchy standard
scope: packages/*/src/**/errors.rb, packages/*/README.md
triggers:
  - adding, renaming, raising, or constructing an error class
  - documenting error classes in README or YARD
  - designing or reviewing a package errors.rb
  - changing rescue targets or the Error marker module
---

# errors

Follow these rules **exactly** when describing, categorizing, and documenting every error class a library raises or constructs for callbacks. Do not invent alternate hierarchies, mix categories, or skip required documentation sections. All paths are relative to the repo root.

Apply this harness whenever designing, implementing, renaming, raising, constructing, or documenting error classes in any package under `packages/*`.
...
|---------|---------------|
| Lean DV hierarchy | `packages/cnpj-dv/src/cnpj-dv/errors.rb` |
| Full Fmt hierarchy (`on_fail` length) | `packages/cnpj-fmt/src/cnpj-fmt/errors.rb` |
| Gen hierarchy | `packages/cnpj-gen/src/cnpj-gen/errors.rb` |
| README documentation shape | `packages/cnpj-gen/README.md` (`## API` errors) |
